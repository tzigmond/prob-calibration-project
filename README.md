...
The loss gradients (the quantities that drive training) are hand-derived and
finite-difference checked in the test suite. SciPy is used only where reinventing
it adds nothing: the Student-*t* log-density's normalizing constant and the
one-off fit of the degrees-of-freedom `ν` and scale `σ` from residuals.
